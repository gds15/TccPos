<?php
	if ( $_POST ) {
           ...
           if (empty($senhaFinal)){
                $sql = "update professor set  login = ?, contato = ? where id = ? limit 1";
                $consulta = $pdo->prepare($sql);
                $consulta->bindParam(1, $login);
                $consulta->bindParam(2, $contato);
                ...
                $sql = "update professor set  login = ?, senha = ?, contato = ? where id = ? limit 1";
                $consulta = $pdo->prepare($sql);
                $consulta->bindParam(1, $login);
                $consulta->bindParam(2, $senhaFinal);
                $consulta->bindParam(3, $contato);
